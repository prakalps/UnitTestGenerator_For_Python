...
class AITestRunner:
    ...
    def run(self) -> int:
        ...
        if self.config.dry_run:
            self._print_summary(
                ...
                coverage,
                generated_count=0,
                validation=ValidationResult(success=True, output="Dry run", skipped=True),
            )
            return 0
    ...
    def _print_summary(
        ...
        if validation.skipped:
            status = "⚠ Tests not run (pytest unavailable)"
        else:
            status = "✔ All tests passing" if validation.success else "✖ Test validation failed"
        print(status)
